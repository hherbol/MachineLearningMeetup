...
def select_action(state):
    ...
    if sample > eps_threshold:
        with torch.no_grad():
            ...
            blarg = policy_net(state)
            return blarg.max(1)[1].view(1, 1)
    else:
        return torch.tensor(
...
def optimize_model():
    ...
    non_final_next_states = torch.cat([s for s in batch.next_state
                                                if s is not None])
    # batch.states holds 128 states, each of 1 dim, each with 3 values hold
    # The values are 40x40x40, each of 90.  I assume this is the RGB?
    ...
    blarg = policy_net(state_batch)
    print(reward_batch)
    state_action_values = blarg.gather(1, action_batch)
